...
int main(int argc, char *argv[])
{
    ...
    std::string jsonfilename = "non-homogeneous-stretch3D-TH.json";
    if (argc > 1)
...
TPZCompMesh *CreateCMeshU(ProblemData *simData, TPZGeoMesh *gmesh)
{
    ...
    if (simData->DomainVec().size() != 0)
    {
        ...
        for (const auto &domain : simData->DomainVec())
        {
            auto *mat_normal = new TPZNullMaterial<>(domain.matID);
            mat_normal->SetNStateVariables(simData->Dim());
            cmesh_u->InsertMaterialObject(mat_normal);
            materialIDs.insert(domain.matID);
            ...
            {
                int domainID = bc.domainID;
                if (domainID != domain.matID)
                    continue;
                val2 = bc.value;
...
TPZCompMesh *CreateCMeshP(ProblemData *simData, TPZGeoMesh *gmesh)
{
    ...
    if (simData->DomainVec().size() != 0) // if true, there is a 2D/3D domain
    {
        ...
        for (const auto &domain : simData->DomainVec())
        {
            auto *mat = new TPZNullMaterial<>(domain.matID);
...
TPZMultiphysicsCompMesh *CreateMultiphysicsMesh(ProblemData *simData, TPZGeoMesh *gmesh, TPZAnalyticSolution* sol)
{
    ...
    if (simData->DomainVec().size() != 0)
    {
        ...
        {
            REAL young = domain.E;
            REAL poisson = domain.nu;
            TPZElasticityTH::AnalysisType type = TPZElasticityTH::AnalysisType::EGeneral;
            if (dynamic_cast<TElasticity2DAnalytic*>(sol))
            {
                ...
                type = (elas->fPlaneStress)? TPZElasticityTH::AnalysisType::EPlaneStress : TPZElasticityTH::AnalysisType::EPlaneStrain;
            ...
            else if (dynamic_cast<TElasticity3DAnalytic*>(sol))
            {
                ...
                type = TPZElasticityTH::AnalysisType::EGeneral;
                if (elas->fProblemType == TElasticity3DAnalytic::ENone) sol = nullptr;
            }
           
            TPZElasticityTH *mat = new TPZElasticityTH(domain.matID, simData->Dim(), young, poisson, type);
